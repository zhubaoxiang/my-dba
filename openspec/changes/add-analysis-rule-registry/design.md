...
| 规则管理接口被任意调用 | 全局分析行为被篡改 | 沿用 `AnyLogin`（与基线一致，由 BSA 平台控制）；若需要收紧见 Open Questions |
...
4. 规则管理接口的权限：沿用 `AnyLogin`（与基线一致、由 BSA 平台控制），还是因为「会改动全局分析行为」而收紧到管理员？收紧会引入基线中不存在的 Token 依赖问题（参见 `add-datasource-catalog` 的 D7）

...
### D3: 进度事件由工具回调上报，不解析框架内部事件

- **Decision**：`build_tools()` 增加 `emit` 回调参数，各工具在自己开头上报一句面向使用者的进度文案。
- **Why**：
  - 工具是按请求构造的，且**已有 `collected` 闭包**用于收集来源；再加一个 `emit` 回调是对称的、局部化的改动，不扩散到编排层
  - 文案由工具自己决定，改文案不需要理解 langgraph 的事件模型
  - 避免依赖 `astream_events` 的事件名与过滤规则——该 API 在 langchain 1.x 各版本间差异较多
- **Alternatives considered**：`astream_events` v2（事件语义最全，但要按 name/tag 过滤、事件量大且版本敏感）；从 `updates` 事件反推工具（拿不到「正在进行」的时机，只能拿到「已完成」）
- **约束**：文案 MUST 面向使用者，MUST NOT 包含工具入参或原始返回内容。
...
- **Decision**：参数非法、会话不存在、未配置对话/嵌入模型、向量服务不可用等**在写出第一个字节之前**就能判定的失败，仍返回 `{code, message, data}` 与对应 HTTP 状态码（4000 / 4004 / 4017 / 5000）。只有**成功回答的正文**脱离统一格式。
- **Why**：
  - 那时 HTTP 状态码仍然有意义，404 就是 404，运维与排查不受损
  - 前端只需判一次 `Content-Type`，就能把响应交给既有 axios 通道（错误提示复用现成拦截器）或流式解析器
  - 「接口不遵循统一规范」的范围被压到最小——只有正文流不遵循
- **Alternatives considered**：一律 200 + SSE，错误也走事件帧（前端只有一条路径，但失去 HTTP 语义，且「会话不存在」这类问题会被掩盖成 200）
